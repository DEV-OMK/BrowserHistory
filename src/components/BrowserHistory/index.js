...
class BrowserHistory extends Component {
  ...
  onDelete = deleteId => {
    ...
    const filteredList = historyList.filter(history => history.id !== deleteId)
  ...
  render() {
    ...
    const isListEmpty = filteredHistoryList.length === 0

    return (
      <div className="bg-container">
        <Ribbon changeInput={this.changeInput} />
        {!isListEmpty && (
        ...
        {isListEmpty && (
          <div className="empty-container">
            <p className="empty-caption">There is no history to show</p>
          </div>
        )}
      </div>
    )
